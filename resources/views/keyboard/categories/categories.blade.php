    ...
    <div class="blockquote text-center">
        ...
        <div class="d-flex justify-content-center">
            @foreach ($categories as $category)
              <div class="card mx-2 px-3 h-50" style="width: 18rem;">
                <div class="card-body">
                  <a href="{{ route('keyboards', $category->id) }}" class="card-text">{{$category->name}}</a>
                </div>
                <img class="card-img-top w-100 h-75" src="{{ asset('/storage/'. $category->image)}}" alt="{{$category->image}}">
                  <div class="d-flex justify-content-between btn-group-sm py-2">
